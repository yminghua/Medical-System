...
class FibHeap {
    ...
    public:
        ...
        // Print registration information for all nodes
        void print_reg(FibNode<T> *node, FibNode<T> *prev, int direction);
        void print_reg();
};
 ...
 */
template <class T>
void FibHeap<T>::print_reg(FibNode<T> *node, FibNode<T> *prev, int direction)
{
    FibNode<T> *start=node;
    ...
    {
        if (direction == 1)
            node->registration->print();
        else
            node->registration->print();

        if (node->child != NULL)
            print_reg(node->child, node, 1);
...
void FibHeap<T>::print_reg()
{
    ...
    do {
        i++;
        p->registration->print();
        
        print_reg(p->child, p, 1);
        p = p->right;
    } while (p != min);
    cout << endl;
}
